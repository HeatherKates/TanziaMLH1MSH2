...
saveRDS(dbaObj,file="../results/01_diffbind/MSH2_dbaObj.RDATA")
# Generate and inspect the report of differentially bound regions
diffPeaks <- dba.report(dbaObj,th=1)
write.csv(diffPeaks, file = "../results/01_diffbind/MSH2_differential_binding_results.csv")
diffPeaksIn <- read.csv("../results/01_diffbind/MSH2_differential_binding_results.csv")

#Let's try just subtracting KO peaks from R4 peaks:
...
export(unique_r4_peaks, "../results/01_diffbind/MSH2_unique_peaks_results.bed")
...
write_peaks_to_bed(diffPeaksIn, "../results/01_diffbind/MSH2_differential_binding_results.bed")

# MSH2R4 unique peaks

# Set thresholds
min_Conc_MSH2R4 <- 3  # Adjust this value based on data and expectations
max_Conc_MSH2KO <- 0.1  # Adjust based on definition of "low" in KO

MSH2R4_filtered_peaks <- diffPeaksIn 

# Write the results
write.csv(MSH2R4_filtered_peaks,"../results/01_diffbind/MSH2R4_filtered_differential_binding_results.csv")
#write_peaks_to_bed(MSH2R4_filtered_peaks, "../results/01_diffbind/MSH2R4_filtered_differential_binding_results.bed")
...
peaks <- readPeakFile("../results/01_diffbind/MSH2R4_filtered_differential_binding_results.bed")
...
#Read in DE results
DE_results <- read.csv("../../../RNAseq/results/4-6_deseq2/MSH2_DESeq2_gene_results.csv")
...
write_xlsx(dfs, "../results/01_diffbind/MSH2R4_peaks.xlsx")
